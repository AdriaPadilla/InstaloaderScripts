...
def to_xlsx(file):
	...
	with open(file) as f:
		...
		try:
			...
			info["post_text"] = "null"
		try:
			info["account_id"] = data["node"]["owner"]["id"]
		except KeyError:
		...
		try:
			...
			info["timestamp"] = datetime.fromtimestamp(dt) # convert timestamp to date time
		except KeyError:
			info["timestamp"] = "null"
		try:
...
json_files = glob.glob("your_path_to_jsons/*.json")
...
for json_file in json_files:
	...
	account_id_list.append(info["account_id"])
...
df.to_excel("your_filename.xlsx")
